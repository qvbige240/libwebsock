...
main(int argc, char *argv[])
{
  ...
  if(argc != 4) {
    fprintf(stderr, "Usage: %s <port to listen on> <private key> <ca>\n\nNote: You must be root to bind to port below 1024\n", argv[0]);
    exit(0);
  }
  ctx = libwebsock_init();
  if(ctx == NULL) {
  ...
  libwebsock_bind_ssl(ctx, "0.0.0.0", argv[1], argv[2], argv[3]);
  fprintf(stderr, "libwebsock listening on port %s, private key %s, ca %s \n", argv[1], argv[2], argv[3]);
